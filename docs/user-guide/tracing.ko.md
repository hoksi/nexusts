# 분산 추적 · `nexusjs/tracing` (Tier 2 v0.4)

> English: [`tracing.md`](./tracing.md)
> v0.3 격차 분석의 Tier 2 격차, **v0.4**에서 해소.

`nexusjs/tracing`은 [OpenTelemetry](https://opentelemetry.io/) API의 얇고 관용적인 래퍼:
...
```ts
import { Trace } from 'nexusjs/tracing';
...
```ts
import { withSpan } from 'nexusjs/tracing';
...
```ts
import { getTracingService } from 'nexusjs/tracing';
...
import { tracingMiddleware, TracingService } from 'nexusjs/tracing';
...
interface TracingConfig {
  serviceName?: string;          // default: "nexusjs"
...
OTel SDK 패키지는 (~5MB) 클 수 있으므로, \`nexusjs\`의 **optional peer dep**임. \`nexusjs/tracing\`을 사용하지 않는 앱은 비용을 지불하지 않음.
...
import { TracingService, withSpan } from 'nexusjs/tracing';
