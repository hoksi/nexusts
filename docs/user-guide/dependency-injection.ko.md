...
NexusJS는 NestJS 스타일의 의존성 주입을 사용합니다. 서비스, 리포지토리, 어댑터는 `@Module({ providers, exports })`을 통해 연결되며, 생성 시점에 자동으로 해석됩니다.
...
따라서 NexusJS는 각 파라미터의 **명시적 `@Inject(Token)`**을 표준으로 채택했습니다. 이는 `tsc`, `ts-node`, Bun, Deno 모두에서 휴대 가능하게 만듭니다.
