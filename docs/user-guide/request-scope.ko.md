...
\`\`\`ts
import { Inject, Injectable, REQUEST } from 'nexus';
...
\`\`\`ts
import { getRequest, getRequestScope, getRequestState, setRequestState } from 'nexus';
...
import { requestScopeMiddleware } from 'nexus/core';
...
## 6. 트랜잭션 (\`nexus/drizzle\` 동반)

요청 스코프 DI는 DB 트랜잭션과 자연스럽게 맞물린다. \`nexus/drizzle\`의 \`db.transaction(fn)\`은 트랜잭션 내부에서 \`fn\` 실행; 요청 스코프 \`Tx\` provider와 결합해 같은 트랜잭션을 요청의 모든 서비스에 공유.

(실제 Tx plumbing은 아직 출시되지 않음 — 예시. \`nexus/drizzle\`이 실제 트랜잭션 경계를 소유.)
...
import { DIContainer } from 'nexus/core';
import { Injectable, Inject } from 'nexus/core';
import { requestScopeMiddleware } from 'nexus/core';
import { getRequestScope } from 'nexus/core';
