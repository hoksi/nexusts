# nexusjs/grpc — gRPC 통합

> English version: [`grpc.md`](./grpc.md)

`nexusjs/grpc`은 TypeScript 클래스와 decorator로 gRPC 서비스를 정의하고
...
- **Streaming.** server-streaming, client-streaming, bidirectional
  streaming이 gRPC에서 first-class입니다. (Streaming은 `nexusjs/grpc`의
  v2에서 예정.)
...
둘 다 `nexusjs/grpc`의 `package.json`에서 **optional** peer dependency로
...
`nexusjs/grpc`은 등록한 서비스에 대한 typed client도 만들어줍니다.
