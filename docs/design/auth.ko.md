...
직접 작성한 crypto, 비밀번호 해싱, OAuth 댄스, WebAuthn 의식 없이도 세션, JWT, OAuth, 패스키 인증을 즉시 제공한다. `@nexusts/auth` 모듈은 [`better-auth`](https://www.better-auth.com/)를 감싼다 — Hono 통합을 일급으로 지원하는 TypeScript 네이티브 auth 라이브러리 — 그리고 NexusTS의 DI / 데코레이터 모델에 맞춘다.
...
1. 사용자의 `AuthConfig`(NexusTS 형상)를 better-auth 옵션 객체로 변환.
...
`@nexusts/auth`는 `package.json`에서 **별도 진입점**이다.
...
- **계정 연결 UI** — better-auth가 엔드포인트 노출; NexusTS 컨트롤러로 래핑.
- **Rate limiting** — better-auth의 hook을 래핑.
- **감사 로그** — better-auth가 이벤트 emit; NexusTS 이벤트 시스템에 노출 (v0.2).
