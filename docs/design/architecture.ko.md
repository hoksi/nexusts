...
NexusJS는 다음 네 가지 원칙을 중심으로 설계된 **Bun 네이티브 풀스택 프레임워크**입니다.
...
사용자가 사용하는 모든 표면은 플랫폼 어댑터 **위**에 구현되어 있으므로, 애플리케이션 코드를 변경하지 않고도 어댑터를 교체할 수 있습니다 (예: Drizzle를 Prisma로 교체).

---

## 3. 모듈 트리

NexusJS 앱은 `@Module` 노드의 트리입니다. 루트 모듈은 `new Application(...)`에 전달되며, 스캐너가 import 그래프를 순회하면서 모듈당 하나의 `ApplicationContainer`를 생성합니다.
...
| 서브 경로 | 용도 |
| -------- | ------- |
| `nexusjs/view` | 뷰 엔진 (기본 `RenduAdapter`) |
| `nexusjs/view/inertia` | Inertia 어댑터 + 헬퍼 |
| `nexusjs/view/inertia/ssr` | React/Vue/Svelte/Solid SSR 어댑터 |
| `nexusjs/orm` | ORM 어댑터 (현재 Drizzle) |
| `nexusjs/runtime` | 런타임 어댑터 |

공개 진입점(`nexusjs`)은 안정적이고 합의된 표면만 다시 export합니다. 그 외는 모두 **고급**이며 메이저 버전 bump 없이 변경될 수 있습니다.
...
| 공개 표면은 **작게 유지** | 실험적인 것은 `nexusjs/<x>` 서브 경로 뒤로. |
