...
이 문서는 `@kabyeon/nexusjs/grpc`의 아키텍처를 설명합니다:
...
gRPC 서버 런타임은 약 300KB(최소화 기준)입니다. `@kabyeon/nexusjs`를
...
`@kabyeon/nexusjs/grpc`는 `package.json`의 **별도 진입점**입니다:
