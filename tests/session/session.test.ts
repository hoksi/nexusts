...
describe('encodeSessionCookie / decodeSessionCookie', () => {
	...
	it('round-trips a record', () => {
		...
		expect(decoded?.expiresAt.toISOString()).toBe(record.expiresAt.toISOString());
	});
	...
	it('returns null on the wrong secret', () => {
		const cookie = encodeSessionCookie(record, SECRET);
		expect(decodeSessionCookie(cookie, 'wrong-secret-' + SECRET.slice(12))).toBeNull();
	});
...
describe('AuthService + SessionService binding', () => {
	...
	it('binds the SessionService and reads through getSession()', async () => {
		...
		const cookieValue = (sessions.storage as CookieSessionStorage).encode(session);
...
describe('@CurrentSession decorator', () => {
	...
	it('returns a ParameterDecorator with no options', () => {
		const deco = CurrentSession();
		expect(typeof deco).toBe('function');
	});

	it('accepts all option shapes', () => {
		expect(() => CurrentSession({ required: true })).not.toThrow();
		expect(() => CurrentSession({ touch: true })).not.toThrow();
		expect(() =>
			CurrentSession({ assert: (_s: { id: string }) => true }),
		).not.toThrow();
	});
